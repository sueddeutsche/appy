...
Appy's `local` auth strategy now supports storing users in MongoDB. The rule is very simple: you must have a MongoDB collection with `username` and `password` properties, and the password property must contain a hashed password as generated by the [password-hash](https://npmjs.org/package/password-hash) npm module, or one compatible with the function you supply as the `verify` option to the `local` strategy. Plaintext passwords are quite deliberately NOT supported.
